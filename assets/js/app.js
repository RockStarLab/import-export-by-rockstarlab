    ...
    if (contentType === 'woo_product') {
      return [{
        ...
        options: [{
          ...
          value: 'variations',
          label: window.aieData.i18n.fieldVariations || 'Variations (JSON)',
          type: 'json'
        }]
      }, {
...
var FunctionLibrary = {
  ...
  renderSnippets: function renderSnippets() {
    ...
    var allowedPages = ['amplified-import-export', 'wp-aie-export', 'wp-aie-content-sync', 'wp-aie-functions' // Add Functions page
    ];
  ...
  previewSnippet: function previewSnippet(snippetKey) {
    ...
    if (useButton) {
      ...
      var allowedPages = ['amplified-import-export', 'wp-aie-export', 'wp-aie-content-sync'];
...
var ImportModule = {
  ...
  bindEvents: function bindEvents() {
    ...
    $wizard.on('click', '#aie-select-file', function () {
      $wizard.find('#aie-file-input').trigger('click');
    });
    $wizard.on('change', '#aie-file-input', function (e) {
      return _this.onFileSelect(e);
    });
    $wizard.on('click', '.aie-remove-file', function () {
      return _this.removeFile();
    });

    // Drag & drop (delegated for resilience).
    $wizard.on('dragover', '#aie-upload-area', function (e) {
      e.preventDefault();
      jQuery(e.currentTarget).addClass('aie-dragover');
    });
    $wizard.on('dragleave', '#aie-upload-area', function (e) {
      jQuery(e.currentTarget).removeClass('aie-dragover');
    });
    $wizard.on('drop', '#aie-upload-area', function (e) {
      e.preventDefault();
      var $dropZone = jQuery(e.currentTarget);
