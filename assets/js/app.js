...
var ExportModule = (_ExportModule = {
  ...
  getFilterFieldsByContentType: function getFilterFieldsByContentType(contentType) {
    ...
    return allFields.filter(function (group) {
      ...
      var featuredImageLabel = window.aieData.i18n.fieldGroupFeaturedImage || 'Featured Image';
...
var ImportModule = {
  ...
  handleFile: function handleFile(file) {
    ...
    if (!allowedExtensions.includes(fileExt)) {
      _utils__WEBPACK_IMPORTED_MODULE_1__["default"].showNotice(aieData.i18n.invalidFileTypeCsvJson || 'Invalid file type. Please upload CSV or JSON files only.', 'error');
      return;
    }
  ...
  bindFunctionModalEvents: function bindFunctionModalEvents(sourceIndex, targetField) {
    ...
    jQuery('.aie-create-new-function').on('click', function (e) {
      e.preventDefault();
      _utils__WEBPACK_IMPORTED_MODULE_1__["default"].showNotice(aieData.i18n.createFunctionsInLibrary || 'Creating custom functions will be available in the Functions Library section', 'info');
    });
  ...
  getTargetFields: function getTargetFields(contentType) {
    var fields = {
      ...
      media: [{
        ...
        value: 'file_url',
        label: window.aieData.i18n.fieldFileUrl || 'File URL'
      }, {
        value: 'alt_text',
          ...
          switch (_context5.prev = _context5.next) {
            ...
            case 6:
              data = {
                ...
                mapping: _this13.getFieldMapping(),
                options: {
              ...
              if (contentType === 'database_table') {
                data.options.table_name = _this13.selectedTableName || jQuery('#aie-import-table-name').val();
              }
              _context5.next = 11;
  ...
  startBatchProcessing: function startBatchProcessing() {
    var _this14 = this;
    return _asyncToGenerator( /*#__PURE__*/_babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default().mark(function _callee6() {
      var _response$result, response, progressData;
      return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default().wrap(function _callee6$(_context6) {
        while (1) {
          switch (_context6.prev = _context6.next) {
            case 0:
            ...
            case 3:
              ...
              progressData = {
                ...
                total: ((_response$result = response.result) === null || _response$result === void 0 ? void 0 : _response$result.total) || 0,
                estimates: {
                  elapsed_formatted: '',
                  remaining_formatted: '',
                  items_per_second: 0
              ...
              if (response.completed) {
                // Import completed
                if (response.result) {
                  _this14.onImportComplete(response);
                } else {
                  _this14.onImportFailed(response);
                }
              } else {
                // Process next batch
                setTimeout(function () {
  ...
  onImportComplete: function onImportComplete(response) {
    ...
    if (jobData.started_at && jobData.completed_at) {
      var start = new Date(jobData.started_at);
      var end = new Date(jobData.completed_at);
  ...
  loadYoastFields: function loadYoastFields(contentType) {
    ...
    jQuery.ajax({
      ...
      success: function success(response) {
        if (response.success && response.data.fields && response.data.fields.length > 0) {
          _this17.renderYoastFields(response.data.fields);
        }
      },
  ...
  populateUniqueFieldOptions: function populateUniqueFieldOptions() {
    ...
    uniqueFields.forEach(function (field) {
      var label = _this18.getFieldLabel(field);
    ...
    $select.off('change.uniquefield').on('change.uniquefield', function () {
      _this18.toggleStartImportButton();
    });
  },
  /**
