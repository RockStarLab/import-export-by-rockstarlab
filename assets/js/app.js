...
var AIURLImporter = {
  ...
  bindEvents: function bindEvents() {
    ...
    jQuery('#aie-csv-file-input').on('change', function (e) {
      return self.handleCSVUpload(e);
    });
    jQuery('.aie-remove-file').on('click', function () {
      return self.removeCSVFile();
    });
   ...
   */
  handleCSVUpload: function handleCSVUpload(e) {
    var _this2 = this;
   ...
   * Remove CSV file
   */
  removeCSVFile: function removeCSVFile() {
...
var ContentSyncModule = {
  /**
   * Initialize module
   	sites.forEach((site) => {				<td class="column-last-sync">
  				${lastSync}
  			</td>
  			<td class="column-actions">
  				<button type="button" class="button button-small aie-test-connection" data-site-id="${site.id}" title="${window.aieData.i18n.testConnection}">
  					<span class="dashicons dashicons-update"></span>
  				</button>
  				<button type="button" class="button button-small aie-edit-site" data-site-id="${site.id}" title="${window.aieData.i18n.edit}">
  					<span class="dashicons dashicons-edit"></span>
  				</button>
  				<button type="button" class="button button-small aie-delete-site" data-site-id="${site.id}" title="${window.aieData.i18n.delete}">
  					<span class="dashicons dashicons-trash"></span>
  				</button>
  			</td>sClass = `aie-status-${site.status}`;
  	const lastSync = site.last_sync_at
  		? new Date(site.last_sync_at).toLocaleString()
  		: window.aieData.i18n.never;
  		const row = `t() {
  ...
  renderSites: function renderSites(sites) {
    ...
    if (!sites || sites.length === 0) {
      $tbody.html("\n\t\t\t\t<tr class=\"aie-no-sites\">\n\t\t\t\t\t<td colspan=\"5\" style=\"text-align: center; padding: 40px;\">\n\t\t\t\t\t\t<span class=\"dashicons dashicons-admin-site\" style=\"font-size: 48px; opacity: 0.3;\"></span>\n\t\t\t\t\t\t<p style=\"margin-top: 40px\">".concat(window.aieData.i18n.noConnectedSites, "</p>\n\t\t\t\t\t</td>\n\t\t\t\t</tr>\n\t\t\t"));
      return;
    }
    sites.forEach(function (site) {
      var statusClass = "aie-status-".concat(site.status);
      var lastSync = site.last_sync_at ? new Date(site.last_sync_at).toLocaleString() : window.aieData.i18n.never;
      var row = "\n\t\t\t\t<tr data-site-id=\"".concat(site.id, "\">\n\t\t\t\t\t<td class=\"column-name\">\n\t\t\t\t\t\t<strong>").concat(_this3.escapeHtml(site.name), "</strong>\n\t\t\t\t\t</td>\n\t\t\t\t\t<td class=\"column-url\">\n\t\t\t\t\t\t<a href=\"").concat(_this3.escapeHtml(site.remote_url), "\" target=\"_blank\" rel=\"noopener noreferrer\">\n\t\t\t\t\t\t\t").concat(_this3.escapeHtml(site.remote_url), "\n\t\t\t\t\t\t</a>\n\t\t\t\t\t</td>\n\t\t\t\t\t<td class=\"column-status\">\n\t\t\t\t\t\t<span class=\"aie-status-badge ").concat(statusClass, "\">\n\t\t\t\t\t\t\t").concat(_this3.escapeHtml(site.status), "\n\t\t\t\t\t\t</span>\n\t\t\t\t\t</td>\n\t\t\t\t\t<td class=\"column-last-sync\">\n\t\t\t\t\t\t").concat(lastSync, "\n\t\t\t\t\t</td>\n\t\t\t\t\t<td class=\"column-actions\">\n\t\t\t\t\t\t<button type=\"button\" class=\"button button-small aie-test-connection\" data-site-id=\"").concat(site.id, "\" title=\"").concat(window.aieData.i18n.testConnection, "\">\n\t\t\t\t\t\t\t<span class=\"dashicons dashicons-update\"></span>\n\t\t\t\t\t\t</button>\n\t\t\t\t\t\t<button type=\"button\" class=\"button button-small aie-edit-site\" data-site-id=\"").concat(site.id, "\" title=\"").concat(window.aieData.i18n.edit, "\">\n\t\t\t\t\t\t\t<span class=\"dashicons dashicons-edit\"></span>\n\t\t\t\t\t\t</button>\n\t\t\t\t\t\t<button type=\"button\" class=\"button button-small aie-delete-site\" data-site-id=\"").concat(site.id, "\" title=\"").concat(window.aieData.i18n["delete"], "\">\n\t\t\t\t\t\t\t<span class=\"dashicons dashicons-trash\"></span>\n\t\t\t\t\t\t</button>\n\t\t\t\t\t</td>\n\t\t\t\t</tr>\n\t\t\t");
      $tbody.append(row);
    });
  },
  /**
  ...
  showAddSiteModal: function showAddSiteModal() {
    var $ = jQuery;
    $('#aie-modal-title').text(window.aieData.i18n.addNewSite);
  ...
  showEditSiteModal: function showEditSiteModal(siteId) {
    ...
    if (!site) return;
    $('#aie-modal-title').text(window.aieData.i18n.editSite);
  ...
  saveSite: function saveSite() {
    ...
    var buttonText = isEdit && !hasApiKey ? window.aieData.i18n.updating : window.aieData.i18n.validatingSaving;
    $saveBtn.prop('disabled', true).text(buttonText);

    // Show info message when validating API key
    if (hasApiKey) {
      this.showModalNotice('info', window.aieData.i18n.validatingApiKey, window.aieData.i18n.pleaseWaitVerifying);
    }
    $.ajax({
      url: ajaxurl,
      type: 'POST',
      ...
      success: function success(response) {
        if (response.success) {
          var message = response.data.message || window.aieData.i18n.operationCompleted;

          // Check if no changes were made
          if (message.includes('No changes')) {
            _this4.showModalNotice('info', window.aieData.i18n.noChanges, message);
            // Close modal after delay
            setTimeout(function () {
              $('#aie-site-modal').hide();
            }, 2000);
          } else {
            _this4.showModalNotice('success', window.aieData.i18n.success, message);
          ...
          _this4.showModalNotice('error', window.aieData.i18n.validationFailed, response.data.message || window.aieData.i18n.failedSaveSiteConnection);
        }
      },
      error: function error(xhr) {
        var errorTitle = window.aieData.i18n.connectionError;
        var errorMessage = window.aieData.i18n.unexpectedError;
        var errorDetails = [];
        ...
        if (xhr.responseJSON) {
          ...
          if (errorMessage.includes('Cannot connect to remote site')) {
            errorTitle = window.aieData.i18n.connectionFailed;
            errorDetails.push(window.aieData.i18n.possibleReasons);
            errorDetails.push(window.aieData.i18n.urlIncorrect);
            errorDetails.push(window.aieData.i18n.remoteSiteOffline);
            errorDetails.push(window.aieData.i18n.networkFirewall);
          } else if (errorMessage.includes('Invalid API key')) {
            errorTitle = window.aieData.i18n.invalidApiKey;
            errorDetails.push(window.aieData.i18n.toResolveIssue);
            errorDetails.push(window.aieData.i18n.goToContentSync);
            errorDetails.push(window.aieData.i18n.clickShowDetails);
            errorDetails.push(window.aieData.i18n.copyEntireKey);
          } else if (errorMessage.includes('plugin is not installed') || errorMessage.includes('plugin is not active')) {
            errorTitle = window.aieData.i18n.pluginNotFound;
            // No additional details needed, message is clear
          } else if (errorMessage.includes('already connected')) {
            errorTitle = window.aieData.i18n.duplicateConnection;
            errorDetails.push(window.aieData.i18n.siteAlreadyConnected);
          } else if (errorMessage.includes('required')) {
            errorTitle = window.aieData.i18n.validationError;
            // Field validation errors are clear enough
          } else {
            errorTitle = window.aieData.i18n.error;
            // Use the server message as-is for other errors
          }
        } else if (xhr.status === 0) {
          errorTitle = window.aieData.i18n.networkError;
          errorMessage = window.aieData.i18n.unableConnectServer;
        } else if (xhr.status >= 500) {
          errorTitle = window.aieData.i18n.serverError;
          errorMessage = window.aieData.i18n.serverReturnedError.replace('%s', xhr.status);
        } else if (xhr.status === 404) {
          errorTitle = window.aieData.i18n.notFound;
          errorMessage = window.aieData.i18n.endpointNotFound;
        }
        _this4.showModalNotice('error', errorTitle, errorMessage, errorDetails);
      },
      complete: function complete() {
        $saveBtn.prop('disabled', false).text(window.aieData.i18n.saveConnection);
      }
    });
  },
  /**
  ...
  toggleMySiteInfo: function toggleMySiteInfo() {
    ...
    $info.slideToggle();
    if ($info.is(':visible')) {
      $btn.html("<span class=\"dashicons dashicons-hidden\"></span> ".concat(window.aieData.i18n.hideDetails));
    } else {
      $btn.html("<span class=\"dashicons dashicons-visibility\"></span> ".concat(window.aieData.i18n.showDetails));
    }
  },
  /**
  ...
  copyMyApiKey: function copyMyApiKey() {
    ...
    $btn.html("<span class=\"dashicons dashicons-yes\"></span> ".concat(window.aieData.i18n.copied));
    setTimeout(function () {
  ...
  regenerateMyApiKey: function regenerateMyApiKey() {
    ...
    var originalText = $btn.html();
    $btn.prop('disabled', true).html("<span class=\"dashicons dashicons-update\"></span> ".concat(window.aieData.i18n.regenerating));
    $.ajax({
      url: ajaxurl,
      type: 'POST',
      data: {
      ...
      success: function success(response) {
        if (response.success) {
          ...
          $btn.html("<span class=\"dashicons dashicons-yes\"></span> ".concat(window.aieData.i18n.regenerated));
          setTimeout(function () {
            $btn.html(originalText);
          }, 3000);
        } else {
...
var ContentUpdater = {
  ...
  getConditionsByFieldType: function getConditionsByFieldType(fieldType) {
    ...
    var dateConditions = [{
      value: 'equals',
      label: window.aieData.i18n.onDate
    }, {
      value: 'before',
      label: window.aieData.i18n.before
    }, {
      value: 'after',
      label: window.aieData.i18n.after
    }, {
      value: 'between',
      label: window.aieData.i18n.between
    }];
    ...
    jQuery.ajax({
      ...
      success: function success(response) {
        if (response.success) {
          ...
          _this34.showNotice(response.data.message || window.aieData.i18n.testFailed, 'error');
        }
      },
      error: function error(xhr, status, _error5) {
        _this34.showNotice(window.aieData.i18n.errorTestingPipeline, 'error');
      }
    });
  },
  /**
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            ...
            case 8:
              ...
              format = _this4.fileData.format || 'csv'; // Update stats
