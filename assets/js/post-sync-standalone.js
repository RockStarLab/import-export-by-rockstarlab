  ...
  var PostSync = {
    ...
    loadMappingData: function loadMappingData(direction, siteId, postIds) {
      ...
      $.ajax({
        ...
        success: function success(localResponse) {
          if (localResponse.success && localResponse.data.posts) {
            // Store local posts info
            _this4.localPostsInfo = {};
            localResponse.data.posts.forEach(function (post) {
              _this4.localPostsInfo[post.ID] = post;
            });
    ...
    createMappingRow: function createMappingRow(postId, remotePosts) {
      ...
      if (this.localPostsInfo && this.localPostsInfo[postId]) {
        ...
        postTitle = $("#post-".concat(postId, " .row-title")).text() || $('.editor-post-title__input').val() || $('#title').val() || $('h1.wp-heading-inline').next('a.page-title-action').prev().text() || postTitle;
        postType = ((_$$attr$match = $('body').attr('class').match(/post-type-(\S+)/)) === null || _$$attr$match === void 0 ? void 0 : _$$attr$match[1]) || postType;
      }
      ...
      setTimeout(function () {
        _this6.initializeSelect2($select, postTitle, postType);
      }, 100);
      return $row;
    },
    /**
      ...
      $select.select2({
        ...
        ajax: {
          ...
          processResults: function processResults(response, params) {
            ...
            var updateTemplate = ((_aiePostSyncData15 = aiePostSyncData) === null || _aiePostSyncData15 === void 0 ? void 0 : (_aiePostSyncData15$i = _aiePostSyncData15.i18n) === null || _aiePostSyncData15$i === void 0 ? void 0 : _aiePostSyncData15$i.updatePost) || '🔄 Update: %1$s (ID: %2$s)';
        ...
        templateSelection: function templateSelection(item) {
          ...
          var $wrapper = $select.closest('.aie-remote-select-wrapper');
          if (item.id === 'new') {
    ...
    autoMatchByTitle: function autoMatchByTitle() {
      ...
      $('.aie-remote-select').each(function (i, select) {
        ...
        var $row = $("tr[data-local-id=\"".concat(localId, "\"]"));
        var localTitle = $row.find('.aie-local-post-info h4').text().trim();
        var localType = $row.find('.aie-post-type').text().trim();
                ...
                for (_iterator.s(); !(_step = _iterator.n()).done;) {
                  ...
                  if (post.post_title.toLowerCase() === localTitle.toLowerCase()) {
                    ...
                    if (!optionExists) {
                      ...
                      var updateTemplate = ((_aiePostSyncData17 = aiePostSyncData) === null || _aiePostSyncData17 === void 0 ? void 0 : (_aiePostSyncData17$i = _aiePostSyncData17.i18n) === null || _aiePostSyncData17$i === void 0 ? void 0 : _aiePostSyncData17$i.updatePost) || '🔄 Update: %1$s (ID: %2$s)';
                      var optionText = updateTemplate.replace('%1$s', post.post_title).replace('%2$s', post.ID);
    ...
    confirmMapping: function confirmMapping() {
      ...
      var postIdsToSync = this.currentPostIds;
      if (this.currentSyncDirection === 'pull') {
        // Extract remote IDs from mapping for pull
        postIdsToSync = [];
        Object.keys(postMapping).forEach(function (localId) {
          var remoteId = postMapping[localId];
    ...
    performSync: function performSync(direction, siteId, postIds, postMapping) {
      ...
      $.ajax({
        ...
        error: function error(xhr) {
          ...
          _this8.showResult('error', errorMessage);
        },
    ...
    loadRemotePosts: function loadRemotePosts() {
      ...
      $.ajax({
        ...
        success: function success(response) {
          if (response.success && response.data && response.data.posts) {
            _this10.renderPostsTree(response.data.posts);
        ...
        error: function error(xhr) {
          ...
          _this10.showBrowseError(errorMessage);
        }
      });
    },
    /**
    ...
    */
    renderPostsTree: function renderPostsTree(posts) {
      ...
      posts.forEach(function (post) {
        var $item = _this11.createPostItem(post);
        $tree.append($item);
      });
      $tree.show();
        ...
        success: function success(response) {
          if (response.success && response.data.children) {
            ...
            response.data.children.forEach(function (child) {
              var $childItem = _this12.createPostItem(child);
