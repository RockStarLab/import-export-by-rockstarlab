...
function wp(env, args, { trim = true } = {}) {
  const out = execFileSync(env.localPhp, [env.wpBin, `--path=${env.source.wpPath}`, ...args], {
    encoding: 'utf8',
...
async function gotoAdminPage(page, env, adminPathWithQuery) {
  await page.goto(`${env.source.baseUrl}${adminPathWithQuery}`, { waitUntil: 'domcontentloaded' });
...
async function waitStep(page, stepNum) {
  await page.waitForSelector(`.aie-step-${stepNum}.active`, { timeout: 45_000 });
}
...
async function clickNextStep(page) {
  ...
  if (!enabled) {
    ...
    throw new Error(`Next Step button stayed disabled (activeStep=${stepClass || 'unknown'})`);
  }
  await next.click();
}
...
async function selectFilterFieldByDataType(page, row, dataType) {
  const ok = await page.evaluate(
    ({ rowSelector, dataTypeWanted }) => {
      ...
      const sel = rowEl.querySelector('select.aie-filter-field');
      if (!sel) return false;
      const opts = Array.from(sel.querySelectorAll('option'));
      const found = opts.find((o) => (o.dataset && o.dataset.type) === dataTypeWanted);
      if (!found) return false;
      sel.value = found.value;
      sel.dispatchEvent(new Event('change', { bubbles: true }));
      return true;
    },
    { rowSelector: await row.evaluate((el) => el.getAttribute('data-test-row') || ''), dataTypeWanted: dataType }
  );
  return ok;
}
...
async function configureRequiredSelectorsOnStep2(page, env, contentType) {
  ...
  if (contentType === 'database_table') {
    ...
    await selectOptionByValueOrText(tableSel, { value: 'wp_users', textRe: /users/i });
...
async function addIdEqualsFilterIfPossible(page, env, { contentType, idValue }) {
  const row = await addFilterRow(page);
  const rowSel = await tagRowForEval(row);
  ...
  await page
    .locator(`${rowSel} select.aie-filter-condition option[value="equals"]`)
    .first()
...
async function configureFormatOnStep4(page, { format, csvDelimiter }) {
  ...
  if (format === 'json') {
    const jsonRadio = page.locator('.aie-step-4.active input[name="format"][value="json"]').first();
    if (await jsonRadio.count()) {
      const label = page.locator('.aie-step-4.active label.aie-format-option', { has: jsonRadio }).first();
      if (await label.count()) {
        await label.click({ force: true });
      } else {
        await page.evaluate(() => {
          const el = document.querySelector('.aie-step-4.active input[name="format"][value="json"]');
          if (!el) return;
          el.checked = true;
          el.dispatchEvent(new Event('change', { bubbles: true }));
          el.dispatchEvent(new Event('click', { bubbles: true }));
    ...
    const csvRadio = page.locator('.aie-step-4.active input[name="format"][value="csv"]').first();
    if (await csvRadio.count()) {
      const label = page.locator('.aie-step-4.active label.aie-format-option', { has: csvRadio }).first();
      if (await label.count()) {
        await label.click({ force: true });
      } else {
        await page.evaluate(() => {
          const el = document.querySelector('.aie-step-4.active input[name="format"][value="csv"]');
          if (!el) return;
          el.checked = true;
          el.dispatchEvent(new Event('change', { bubbles: true }));
          el.dispatchEvent(new Event('click', { bubbles: true }));
  ...
  if (format === 'csv' && csvDelimiter) {
    const delSel = page.locator('.aie-step-4.active select[name="csv_delimiter"]').first();
    if (await delSel.count()) {
      // Options use literal values: ",", ";", "\\t", "|", "custom"
      await delSel.selectOption({ value: csvDelimiter }).catch(() => {});
    }
  }
}
...
async function startExportAndDownload(page, artifactsDir) {
  ...
  await page.locator('.aie-step-4.active .aie-start-export').click();
  ...
  const downloadBtn = page.locator('.aie-download-file').first();
...
function normalizeContentTypeToExpectedQuery({ exportType, customPostType, taxonomy, tableName }) {
  if (exportType === 'post') return { kind: 'post', postType: 'post' };
  if (exportType === 'page') return { kind: 'post', postType: 'page' };
  if (exportType === 'media') return { kind: 'post', postType: 'attachment' };
  if (exportType === 'woo_product') return { kind: 'post', postType: 'product' };
  if (exportType === 'woo_coupon') return { kind: 'post', postType: 'shop_coupon' };
  if (exportType === 'woo_order') return { kind: 'woo_order' };
  if (exportType === 'custom_post_types') return { kind: 'post', postType: String(customPostType || '') };
  if (exportType === 'taxonomy') return { kind: 'term', taxonomy: String(taxonomy || '') };
  ...
  if (exportType === 'database_table') return { kind: 'db', tableName: String(tableName || '') };
  return { kind: 'unknown' };
}
...
`;
  ...
  return out && typeof out.count === 'number' ? out : { count: 0, sample: null };
}
...
function findColumnIndex(headers, candidates) {
  ...
  for (const c of candidates) {
    ...
    const idx = lower.findIndex((h) => h === lc || h.endsWith(`_${lc}`) || h.includes(lc));
    if (idx !== -1) return idx;
  }
  return -1;
}
...
function verifyExportFile({ filePath, format, delimiterValue, expectedCount, sample, exportType }) {
  ...
  if (format === 'json') {
    ...
    try {
      ...
      issues.push(summarizeMismatch({ label: 'json_parse', expected: 'valid json', actual: String(e && e.message ? e.message : e) }));
      return issues;
    }
    const arr = Array.isArray(data) ? data : data && Array.isArray(data.items) ? data.items : null;
    if (!arr) {
      issues.push(summarizeMismatch({ label: 'json_shape', expected: 'array or {items:[]}', actual: typeof data }));
      return issues;
    }
    if (typeof expectedCount === 'number' && expectedCount >= 0 && arr.length !== expectedCount) {
      issues.push(summarizeMismatch({ label: 'count', expected: expectedCount, actual: arr.length }));
    }
    if (sample && sample.id) {
      const idKey = Object.keys(arr[0] || {}).find((k) => String(k).toLowerCase() === 'id') || 'ID';
      const found = arr.find((x) => String(x[idKey] ?? '') === String(sample.id));
      if (!found) issues.push(summarizeMismatch({ label: 'sample_present', expected: `id=${sample.id}`, actual: 'not found' }));
    }
    return issues;
  }
  ...
  if (!rows.length) {
    issues.push(summarizeMismatch({ label: 'csv_rows', expected: '>0', actual: 0 }));
    return issues;
  }

  const headers = rows[0] || [];
  const dataRows = rows.slice(1);
  if (typeof expectedCount === 'number' && expectedCount >= 0 && dataRows.length !== expectedCount) {
    issues.push(summarizeMismatch({ label: 'count', expected: expectedCount, actual: dataRows.length }));
  }

  if (sample && sample.id) {
    const idIdx = findColumnIndex(headers, ['ID', 'id', 'term_id', 'user_id', 'comment_ID', 'attribute_id', 'order_id', 'coupon_id']);
    if (idIdx !== -1) {
      const found = dataRows.find((r) => String(r[idIdx] ?? '') === String(sample.id));
      if (!found) issues.push(summarizeMismatch({ label: 'sample_present', expected: `id=${sample.id}`, actual: 'not found' }));
    }

    // Basic title/name check for known types if columns exist.
    if (exportType === 'post' || exportType === 'page' || exportType === 'custom_post_types' || exportType === 'woo_product' || exportType === 'woo_coupon') {
      const titleIdx = findColumnIndex(headers, ['post_title', 'title']);
      if (titleIdx !== -1 && sample.title) {
        const row = dataRows.find((r) => String(r[titleIdx] ?? '').trim() === String(sample.title).trim());
        if (!row) {
          issues.push(
            summarizeMismatch({
    ...
    if (exportType === 'taxonomy' || exportType === 'menu') {
      ...
      if (nameIdx !== -1 && sample.name) {
        const row = dataRows.find((r) => String(r[nameIdx] ?? '').trim() === String(sample.name).trim());
        if (!row) {
          issues.push(summarizeMismatch({ label: 'sample_name_present', expected: sample.name, actual: 'not found' }));
        }
      }
    }
    if (exportType === 'user') {
      const loginIdx = findColumnIndex(headers, ['user_login', 'login']);
      if (loginIdx !== -1 && sample.login) {
        const row = dataRows.find((r) => String(r[loginIdx] ?? '').trim() === String(sample.login).trim());
        if (!row) {
          issues.push(summarizeMismatch({ label: 'sample_login_present', expected: sample.login, actual: 'not found' }));
        }
      }
    }
  }
...
async function runOneExportCase(page, env, { exportType, label, format, csvDelimiter, addIdFilter }) {
  await resetWizardToStep1(page, env);
  const selected = await selectContentType(page, exportType);
  if (!selected) return { skipped: true, label, exportType, reason: 'disabled-or-missing' };

  await clickNextStep(page); // step 2
  await waitStep(page, 2);

  const required = await configureRequiredSelectorsOnStep2(page, env, exportType);
  ...
  if (exportType === 'custom_post_types' && !required.customPostType) {
    return { skipped: true, label, exportType, reason: 'post-type-not-selected', required };
  }
  if (exportType === 'taxonomy' && !required.taxonomy) {
    return { skipped: true, label, exportType, reason: 'taxonomy-not-selected', required };
  }
  if (exportType === 'database_table' && !required.tableName) {
    return { skipped: true, label, exportType, reason: 'table-not-selected', required };
  }

  // Force refresh after required selector changes (taxonomy/custom post type selectors attach handlers late).
  const refreshBtn = page.locator('.aie-step-2.active .aie-refresh-count').first();
  ...
  await page
    .waitForFunction(() => {
      const el = document.querySelector('.aie-step-2.active .aie-count-value');
      if (!el) return false;
  ...
  if (exportType === 'taxonomy' || exportType === 'database_table') {
    const preQuery = normalizeContentTypeToExpectedQuery({ exportType, ...required });
    const preExpected = getExpectedCountAndSample(env, preQuery);
    if (preExpected && typeof preExpected.count === 'number' && preExpected.count > 0) {
      await page
        .waitForFunction(() => {
          const el = document.querySelector('.aie-step-2.active .aie-count-value');
          if (!el) return false;
  ...
  const countText = (await page.locator('.aie-step-2.active .aie-count-value').innerText().catch(() => '-')).trim();
  const countNum = Number.parseInt(countText, 10);
  if (Number.isFinite(countNum) && countNum === 0) {
    return { skipped: true, label, exportType, reason: 'no-items', required };
  }

  // Optional ID equals filter (if we can derive an ID).
  let idUsed = null;
  if (addIdFilter) {
    const query = normalizeContentTypeToExpectedQuery({ exportType, ...required });
    ...
    if (expected0 && expected0.sample && expected0.sample.id) {
      ...
      if (exportType === 'taxonomy' && expected0.sample.slug) {
        await addSlugEqualsFilter(page, { slugValue: expected0.sample.slug });
      } else {
        await addIdEqualsFilterIfPossible(page, env, { contentType: exportType, idValue: idUsed });
      }
    }
  }
  ...
  const artifactsDir = path.resolve(process.cwd(), 'e2e', 'artifacts', 'aie-export-check');
...
async function main() {
  ...
  try {
    ...
    for (const t of env.types) {
      cases.push({ exportType: t, label: `${t}:csv:default`, format: 'csv', csvDelimiter: ',', addIdFilter: false });
    }

    // Extra coverage: options + filters on key types.
    cases.push({ exportType: 'post', label: 'post:csv:semicolon', format: 'csv', csvDelimiter: ';', addIdFilter: false });
    cases.push({ exportType: 'post', label: 'post:json', format: 'json', csvDelimiter: ',', addIdFilter: false });
    cases.push({ exportType: 'post', label: 'post:csv:filter-id', format: 'csv', csvDelimiter: ',', addIdFilter: true });
    cases.push({ exportType: 'taxonomy', label: 'taxonomy:csv:filter-id', format: 'csv', csvDelimiter: ',', addIdFilter: true });
    cases.push({ exportType: 'custom_post_types', label: 'custom_post_types:csv:filter-id', format: 'csv', csvDelimiter: ',', addIdFilter: true });

    const results = [];
    for (const c of cases) {
      // Skip duplicates when base list doesn't include the type.
      if (!env.types.includes(c.exportType) && !['post', 'taxonomy', 'custom_post_types'].includes(c.exportType)) continue;
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.log(`[export-check] error ${c.label}: ${r.error}`);
      }
      results.push(r);
      if (r.skipped) {
        // eslint-disable-next-line no-console
        console.log(`[export-check] skipped ${c.label}: ${r.reason}`);
        continue;
      }
      if (r.error) {
        // eslint-disable-next-line no-console
      ...
      if (r.issues && r.issues.length) {
        // eslint-disable-next-line no-console
        console.log(`[export-check] issues ${c.label}:\n${JSON.stringify(r.issues, null, 2)}`);
      } else {
        // eslint-disable-next-line no-console
        console.log(`[export-check] ok ${c.label} (expected=${r.expectedCount}) -> ${r.filePath}`);
      }
    }

    const issuesAll = results.filter((r) => !r.skipped && ((r.issues && r.issues.length) || r.error));
    const outDir = path.resolve(process.cwd(), 'e2e', 'artifacts', 'aie-export-check');
    const summaryPath = path.join(outDir, `summary-${new Date().toISOString().replace(/[:.]/g, '-')}.json`);
    ...
    if (issuesAll.length) {
      // eslint-disable-next-line no-console
      console.log(
        `[export-check] FAIL: ${issuesAll.length} case(s) with issues. First 10 labels:\n- ${issuesAll
          .slice(0, 10)
          .map((x) => x.label)
          .join('\n- ')}`
      );
