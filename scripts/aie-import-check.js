...
function loadEnv() {
  ...
  return {
    headless,
    types,
    repeatDedup: String(get('AIE_IMPORT_REPEAT_DEDUP', 'true')).toLowerCase() !== 'false',
    source: {
...
function wp(env, wpPath, args, { trim = true } = {}) {
  const out = execFileSync(env.localPhp, [env.wpBin, `--path=${wpPath}`, ...args], {
    encoding: 'utf8',
...
async function gotoAdminPage(page, site, adminPathWithQuery) {
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      await page.goto(`${site.baseUrl}${adminPathWithQuery}`, { waitUntil: 'domcontentloaded' });
...
async function waitStep(page, stepNum) {
  await page.waitForSelector(`.aie-step-${stepNum}.active`, { timeout: 60_000 });
}
...
async function clickNextStep(page) {
  ...
  if (!ok) {
    const stepClass = await page.locator('.aie-step.active').first().getAttribute('class').catch(() => '');
    throw new Error(`Next Step stayed disabled (active=${stepClass || 'unknown'})`);
  }
  await next.click();
}
...
async function addFilterRow(page) {
  const btn = page.locator('.aie-step-2.active .aie-add-filter').first();
  await btn.waitFor({ state: 'visible', timeout: 30_000 });
  await btn.click();
  const rows = page.locator('.aie-step-2.active .aie-filter-row');
  await rows.last().waitFor({ state: 'visible', timeout: 30_000 });
  return rows.last();
}

async function tagRowForEval(row) {
  const tag = `row-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  await row.evaluate((el, t) => el.setAttribute('data-test-row', t), tag);
...
async function configureRequiredSelectorsOnExportStep2(page, contentType) {
  if (contentType === 'custom_post_types') {
    ...
    await page.locator(`${rowSel} select.aie-filter-field`).selectOption({ value: '_post_type' });
    const postTypeSelect = page.locator(`${rowSel} select.aie-post-type-selector`).first();
    ...
    await page
      .waitForFunction(
        ...
        `${rowSel} select.aie-post-type-selector`,
        { timeout: 60_000 }
      )
      .catch(() => null);
    await selectOptionByValueOrText(postTypeSelect, { value: 'portfolio', textRe: /portfolio/i });
    return { customPostType: await postTypeSelect.inputValue().catch(() => '') };
  }

  if (contentType === 'taxonomy') {
    const row = await addFilterRow(page);
    const rowSel = await tagRowForEval(row);
    await page.locator(`${rowSel} select.aie-filter-field`).selectOption({ value: '_taxonomy' });
    const taxSelect = page.locator(`${rowSel} select.aie-taxonomy-selector`).first();
    ...
    await page
      .waitForFunction(
        ...
        `${rowSel} select.aie-taxonomy-selector`,
        { timeout: 60_000 }
      )
      .catch(() => null);
    await selectOptionByValueOrText(taxSelect, { value: 'category', textRe: /category/i });
    return { taxonomy: await taxSelect.inputValue().catch(() => '') };
  }

  if (contentType === 'database_table') {
    const tableSel = page.locator('.aie-step-2.active #aie-table-name').first();
    await tableSel.waitFor({ state: 'visible', timeout: 60_000 });
    await page.waitForFunction(() => {
      const sel = document.querySelector('.aie-step-2.active #aie-table-name');
    ...
    for (let i = 0; i < count; i++) {
      const opt = options.nth(i);
      const v = String((await opt.getAttribute('value')) || '').trim();
      const t = String((await opt.innerText().catch(() => '')) || '').trim();
      if (!v) continue;
      candidates.push({ v, t });
    }
    const preferred = candidates.find((x) => /otbo|mask/i.test(x.v) || /otbo|mask/i.test(x.t)) || candidates.find((x) => !/wp_users/i.test(x.v));
...
async function addIdEqualsFilter(page, { idValue }) {
  ...
  const fieldSel = page.locator(`${rowSel} select.aie-filter-field`).first();
  await fieldSel.waitFor({ state: 'visible', timeout: 30_000 });
  const preferredValues = ['ID', 'term_id', 'user_id', 'comment_ID', 'attribute_id', 'order_id', 'coupon_id'];
  let selectedValue = '';
  ...
  if (selectedValue) await fieldSel.selectOption({ value: selectedValue });

  const condition = page.locator(`${rowSel} select.aie-filter-condition`).first();
  await condition.waitFor({ state: 'visible', timeout: 30_000 });
  await selectOptionByValueOrText(condition, { value: 'equals', textRe: /equals|=/i });

  const input = page.locator(`${rowSel} input.aie-filter-value, select.aie-filter-value`).first();
...
async function addSlugEqualsFilter(page, { slugValue }) {
  ...
  } else if (await fieldSel.locator('option[value="post_name"]').count()) {
    await fieldSel.selectOption({ value: 'post_name' });
  } else if (await fieldSel.locator('option[value="user_login"]').count()) {
    await fieldSel.selectOption({ value: 'user_login' });
  } else {
    const first = await fieldSel.locator('option[value]').first().getAttribute('value').catch(() => '');
  ...
  const input = page.locator(`${rowSel} input.aie-filter-value`).first();
  await input.waitFor({ state: 'attached', timeout: 30_000 });
...
async function selectAllFieldsOnStep3(page) {
  ...
  const next = page.locator('.aie-step-3.active .aie-next-step');
...
async function exportCsvFromSource(page, sourceSite, contentType, { filter, requiredSelectors } = {}) {
  await gotoAdminPage(page, sourceSite, '/wp-admin/admin.php?page=rsl-ie-export');
  ...
  if (!selected) return { skipped: true, reason: 'type-disabled-or-missing' };

  await clickNextStep(page);
  await waitStep(page, 2);

  const required = requiredSelectors || (await configureRequiredSelectorsOnExportStep2(page, contentType));
...
async function ensureImportDatabaseTableSelected(page, { preferredTable, patterns }) {
  const sel = page.locator('.aie-step-4.active #aie-import-table-name').first();
  await sel.waitFor({ state: 'visible', timeout: 60_000 });
  await page
    .waitForFunction(() => {
      const s = document.querySelector('.aie-step-4.active #aie-import-table-name');
  ...
  const pats = Array.isArray(patterns) ? patterns : String(patterns || '').split(',').map((x) => x.trim()).filter(Boolean);
  for (const p of pats) {
  ...
  const fallback = list.find((x) => x.v && x.v !== 'custom' && !/wp_users/i.test(x.v)) || list.find((x) => x.v && x.v !== 'custom');
...
async function selectUniqueField(page, preferredValues) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const opt = options.nth(i);
    // eslint-disable-next-line no-await-in-loop
    const val = String((await opt.getAttribute('value').catch(() => '')) || '').trim();
    if (!val) continue;
    // eslint-disable-next-line no-await-in-loop
...
async function readMappingStats(page) {
  const mapped = page.locator('.aie-step-4.active .aie-mapped-count').first();
  const total = page.locator('.aie-step-4.active .aie-total-fields').first();
  ...
  const unmapped = await page.evaluate(() => {
    const cards = Array.from(document.querySelectorAll('.aie-step-4.active .aie-field-card'));
    const items = [];
    for (const c of cards) {
      const used = c.classList.contains('used');
      const disabled = c.classList.contains('disabled');
      if (used || disabled) continue;
      const nameEl = c.querySelector('.aie-field-name') || c.querySelector('.aie-field-title') || c;
...
async function readMappingDetails(page) {
  return await page.evaluate(() => {
    const rows = Array.from(document.querySelectorAll('.aie-mapping-row'));
    const mappingRows = rows
      .map((row) => {
        ...
        const sourceCard = sourceIndex ? document.querySelector(`.aie-field-card[data-source-index="${CSS.escape(sourceIndex)}"]`) : null;
        const sourceField = sourceCard ? sourceCard.getAttribute('data-source-field') : null;
        const fieldType = row.getAttribute('data-field-type');
        return {
          sourceIndex: sourceIndex != null ? Number(sourceIndex) : null,
    ...
    const targetFields = Array.from(document.querySelectorAll('.aie-target-field:not(.aie-custom-field-template)'))
      .map((el) => el.getAttribute('data-target-field'))
      .filter(Boolean);

    const sourceFields = Array.from(document.querySelectorAll('.aie-field-card'))
...
async function importIntoTarget(page, targetSite, contentType, filePath, { requiredSelectors, importOptions } = {}) {
  await gotoAdminPage(page, targetSite, '/wp-admin/admin.php?page=rsl-ie-import');
  ...
  await waitStep(page, 2);
  await page.setInputFiles('#aie-file-input', filePath);
  await page.waitForFunction(() => {
  ...
  if (contentType === 'custom_post_types') {
    const postType = String((requiredSelectors && requiredSelectors.customPostType) || '').trim();
    ...
    if (postType) {
      await page
        ...
        .catch(() => null);
      await postTypeSelect.selectOption({ value: postType }).catch(() => null);
    }
    await page.waitForTimeout(300);
    // Wait for target fields to refresh after CPT selection.
    await page.locator('.aie-step-4.active .aie-target-field[data-target-field="post_name"]').first().waitFor({ state: 'attached', timeout: 30_000 }).catch(() => {});
    await page.locator('.aie-step-4.active .aie-target-field[data-target-field="post_author"]').first().waitFor({ state: 'attached', timeout: 30_000 }).catch(() => {});
  }

  if (contentType === 'database_table') {
    const tableName = String((requiredSelectors && requiredSelectors.tableName) || '').trim();
    await ensureImportDatabaseTableSelected(page, { preferredTable: tableName, patterns: ['otbo', 'mask'] });
    await page.waitForTimeout(300);
  }
  ...
  await page
    .waitForFunction(() => {
      const totalEl = document.querySelector('.aie-step-4.active .aie-total-fields');
      const mappedEl = document.querySelector('.aie-step-4.active .aie-mapped-count');
      const total = totalEl ? Number(String(totalEl.textContent || '').trim()) : 0;
      const mapped = mappedEl ? Number(String(mappedEl.textContent || '').trim()) : 0;
      return Number.isFinite(total) && total > 0 && Number.isFinite(mapped);
    })
    .catch(() => null);
  ...
  if (opts.ifExists) {
    const r = page.locator(`.aie-step-5.active input[name="if_exists"][value="${opts.ifExists}"]`).first();
    if (await r.count()) await r.check({ force: true });
  }
  if (opts.ifNotExists) {
    const r = page.locator(`.aie-step-5.active input[name="if_not_exists"][value="${opts.ifNotExists}"]`).first();
    if (await r.count()) await r.check({ force: true });
  }

  if (contentType === 'database_table') {
    await selectUniqueField(page, opts.uniqueFieldPreferred || []);
  } else if (opts.uniqueFieldPreferred && opts.uniqueFieldPreferred.length) {
    await selectUniqueField(page, opts.uniqueFieldPreferred);
  }

  const mediaCheckbox = page.locator('#aie-auto-import-media').first();
  ...
  if (opts.autoImportMedia !== false && opts.mediaDuplicateMode) {
    const r = page.locator(`.aie-step-5.active input[name="media_duplicate_mode"][value="${opts.mediaDuplicateMode}"]`).first();
    if (await r.count()) await r.check({ force: true });
  }
...
function diffAcfSnapshots(expected, actual) {
  ...
  for (const name of Object.keys(actual || {})) {
    if (!(expected && Object.prototype.hasOwnProperty.call(expected, name))) {
      mismatches.push({ field: name, expected: null, actual: actual[name] });
    }
  }
  return mismatches;
}
...
async function run() {
  ...
  const artifactsRoot = path.resolve(process.cwd(), 'e2e', 'artifacts', 'aie-import-check', nowStamp());
  ...
  try {
    const wooActive = isWooActive(env, env.source.wpPath) && isWooActive(env, env.target.wpPath);

    // Prepare a media baseline for dedup checks (featured image of post=1 on source).
    const baselinePostId = 1;
    const baselineAttachmentId = ensureFeaturedImage(env, env.source.wpPath, baselinePostId);
    const baselineHash = baselineAttachmentId ? getAttachmentMd5(env, env.source.wpPath, baselineAttachmentId) : '';

    const targetAttachmentsBefore = countAttachments(env, env.target.wpPath);
    const targetHashBefore = baselineHash ? countAttachmentsByFileHash(env, env.target.wpPath, baselineHash) : 0;
    ...
    if (env.types.includes('post')) {
      cases.push({
        ...
        export: async () => {
          ...
          const refPostId = pickOnePostId(env, env.source.wpPath, 'post');
          fillAllAcfFieldsForPost(env, env.source.wpPath, postId, { refPostId });
          const slugInfo = getPostSlug(env, env.source.wpPath, postId);
          return { filter: { kind: 'id', value: postId }, identity: { kind: 'post-slug', postType: 'post', slug: slugInfo?.post_name || '' }, sourceObjectId: String(postId) };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: ['post_name', 'post_title', 'ID'] },
        resolveTargetObjectId: (identity) => {
          const id = resolvePostIdBySlug(env, env.target.wpPath, { postType: identity.postType, slug: identity.slug });
          return id ? String(id) : '';
        },
        expectAcf: true,
    ...
    if (env.types.includes('page')) {
      cases.push({
        ...
        export: async () => {
          const pageId = pickOnePostId(env, env.source.wpPath, 'page');
          if (!pageId) return { skipped: true, reason: 'no-pages' };
          const refPostId = pickOnePostId(env, env.source.wpPath, 'post');
          fillAllAcfFieldsForPost(env, env.source.wpPath, pageId, { refPostId });
          const info = getPostSlug(env, env.source.wpPath, pageId);
          return { filter: { kind: 'slug', value: info?.post_name || '' }, identity: { kind: 'post-slug', postType: 'page', slug: info?.post_name || '' }, sourceObjectId: String(pageId) };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: ['post_name', 'post_title', 'ID'] },
        resolveTargetObjectId: (identity) => {
          const id = resolvePostIdBySlug(env, env.target.wpPath, { postType: identity.postType, slug: identity.slug });
    ...
    if (env.types.includes('custom_post_types')) {
      cases.push({
        ...
        export: async () => {
          ...
          const portfolioId = pickOnePostId(env, env.source.wpPath, 'portfolio');
          if (!portfolioId) return { skipped: true, reason: 'no-portfolio' };
          const refPostId = pickOnePostId(env, env.source.wpPath, 'post');
          fillAllAcfFieldsForPost(env, env.source.wpPath, portfolioId, { refPostId });
          const info = getPostSlug(env, env.source.wpPath, portfolioId);
          return {
            filter: { kind: 'id', value: portfolioId },
            identity: { kind: 'post-slug', postType: 'portfolio', slug: info?.post_name || '' },
    ...
    if (env.types.includes('taxonomy')) {
      cases.push({
        ...
        export: async () => {
          ...
          if (!term || !term.term_id) return { skipped: true, reason: 'no-terms' };
          return {
            filter: { kind: 'slug', value: term.slug },
            identity: { kind: 'term-slug', taxonomy: tax, slug: term.slug },
            sourceObjectId: `term_${term.term_id}`,
          };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: ['slug', 'name', 'term_id'] },
        resolveTargetObjectId: (identity) => {
          const termId = resolveTermIdBySlug(env, env.target.wpPath, { taxonomy: identity.taxonomy, slug: identity.slug });
    ...
    if (env.types.includes('menu')) {
      cases.push({
        ...
        export: async () => {
          ...
          if (!menu || !menu.term_id) return { skipped: true, reason: 'no-menus' };
          return { filter: { kind: 'id', value: menu.term_id }, identity: { kind: 'term-slug', taxonomy: 'nav_menu', slug: menu.slug }, sourceObjectId: `term_${menu.term_id}` };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: ['slug', 'name', 'term_id'] },
        resolveTargetObjectId: (identity) => {
          const termId = resolveTermIdBySlug(env, env.target.wpPath, { taxonomy: identity.taxonomy, slug: identity.slug });
    ...
    if (env.types.includes('user')) {
      cases.push({
        ...
        export: async () => {
          ...
          if (!userId) return { skipped: true, reason: 'no-non-admin-user' };
          const info = getUserLogin(env, env.source.wpPath, userId);
          if (!info || !info.user_login) return { skipped: true, reason: 'user-login-missing' };
          return { filter: { kind: 'slug', value: info.user_login }, identity: { kind: 'user-login', login: info.user_login }, sourceObjectId: `user_${userId}` };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: ['user_login', 'user_email', 'ID'] },
        resolveTargetObjectId: (identity) => {
          const id = resolveUserIdByLogin(env, env.target.wpPath, identity.login);
          return id ? `user_${id}` : '';
        },
        expectAcf: true,
      });
    ...
    if (env.types.includes('media')) {
      cases.push({
        ...
        export: async () => {
          ...
          if (!attId) return { skipped: true, reason: 'no-attachments' };
          return { filter: { kind: 'id', value: attId }, identity: { kind: 'media-id', id: attId }, sourceObjectId: '' };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: ['file_url', 'url', 'filename', 'file_name', 'ID'] },
    ...
    if (wooActive && env.types.includes('woo_product')) {
      cases.push({
        ...
        export: async () => {
          ...
          if (!pid) return { skipped: true, reason: 'no-products' };
          const refPostId = pickOnePostId(env, env.source.wpPath, 'post');
          fillAllAcfFieldsForPost(env, env.source.wpPath, pid, { refPostId });
          const sku = getProductSku(env, env.source.wpPath, pid);
          const info = getPostSlug(env, env.source.wpPath, pid);
          return { filter: { kind: 'id', value: pid }, identity: { kind: 'product-sku', sku: sku || '', fallbackSlug: info?.post_name || '' }, sourceObjectId: String(pid) };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: ['sku', '_sku', 'post_name', 'post_title', 'ID'] },
        resolveTargetObjectId: (identity) => {
          const id = identity.sku ? resolveProductIdBySku(env, env.target.wpPath, identity.sku) : 0;
          if (id) return String(id);
          if (identity.fallbackSlug) {
            const bySlug = resolvePostIdBySlug(env, env.target.wpPath, { postType: 'product', slug: identity.fallbackSlug });
    ...
    if (wooActive && env.types.includes('woo_coupon')) {
      cases.push({
        ...
        export: async () => {
          ...
          return { filter: { kind: 'id', value: cid }, identity: { kind: 'coupon-code', code }, sourceObjectId: String(cid) };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: ['code', 'post_title', 'ID'] },
        resolveTargetObjectId: (identity) => {
          const id = resolveCouponIdByCode(env, env.target.wpPath, identity.code);
          return id ? String(id) : '';
        },
        expectAcf: true,
      });
    ...
    if (wooActive && env.types.includes('woo_order')) {
      cases.push({
        ...
        export: async () => {
          ...
          return { filter: { kind: 'id', value: oid }, identity: { kind: 'order-key', orderKey: key }, sourceObjectId: String(oid) };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: ['order_key', 'order_number', 'ID'] },
        resolveTargetObjectId: (identity) => {
          const id = resolveOrderIdByOrderKey(env, env.target.wpPath, identity.orderKey);
          return id ? String(id) : '';
        },
    ...
    if (wooActive && env.types.includes('woo_attribute')) {
      cases.push({
        type: 'woo_attribute',
        label: 'woo_attribute:attribute_name:update+create',
        export: async () => {
          const aid = pickOneWooAttributeId(env, env.source.wpPath);
          if (!aid) return { skipped: true, reason: 'no-attributes' };
          const name = getWooAttributeName(env, env.source.wpPath, aid);
          if (!name) return { skipped: true, reason: 'attribute-name-missing' };
          return { filter: { kind: 'id', value: aid }, identity: { kind: 'attribute-name', name }, sourceObjectId: '' };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: ['attribute_name', 'name', 'slug', 'attribute_id', 'ID'] },
        resolveTargetObjectId: (identity) => {
          const id = resolveWooAttributeIdByName(env, env.target.wpPath, identity.name);
          return id ? String(id) : '';
        },
        expectAcf: false,
      });
    ...
    if (env.types.includes('comment')) {
      cases.push({
        ...
        export: async () => {
          ...
          if (!cid) return { skipped: true, reason: 'no-comments' };
          return { filter: { kind: 'id', value: cid }, identity: { kind: 'comment-id', id: cid }, sourceObjectId: '' };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: ['comment_ID', 'comment_author_email', 'comment_date_gmt'] },
        resolveTargetObjectId: () => '',
        expectAcf: false,
      });
    }
    ...
    if (env.types.includes('database_table')) {
      cases.push({
        ...
        export: async () => {
          ...
          return { filter: null, identity: { kind: 'db-table' }, sourceObjectId: '' };
        },
        importOptions: { ifExists: 'update', ifNotExists: 'create', autoImportMedia: true, mediaDuplicateMode: 'skip', uniqueFieldPreferred: [] },
        resolveTargetObjectId: () => '',
        expectAcf: false,
      });
    }
    ...
    for (const c of cases) {
      const caseDir = path.join(artifactsRoot, slugify(c.type), slugify(c.label));
      mkdirp(caseDir);
      ...
      try {
        ...
        if (prep && prep.skipped) {
          ...
          result.errors.push({ kind: 'skipped', reason: prep.reason || 'skipped' });
        ...
        // Export from source.
        const exp = await exportCsvFromSource(sourcePage, env.source, c.type, {
        ...
        if (exp && exp.skipped) {
          result.skipped = true;
          result.status = 'skipped';
          result.errors.push({ kind: 'skipped', reason: exp.reason || 'export-skipped' });
          summary.results.push(result);
          // eslint-disable-next-line no-continue
          continue;
        }
        ...
        const exportPath = path.join(caseDir, `export-${suggested}`);
        ...
        // Import into target.
        const importRes = await importIntoTarget(targetPage, env.target, c.type, exportPath, {
          requiredSelectors: exp.required || exportInfo.requiredSelectors || {},
          importOptions: c.importOptions || {},
        });
        result.import = { mapping: importRes.mappingStats || null, options: c.importOptions || {} };
        if (importRes.mappingDetails) {
          fs.writeFileSync(path.join(caseDir, 'mapping-details.json'), JSON.stringify(importRes.mappingDetails, null, 2));
        }

        // Check mapping completeness.
        if (importRes.mappingStats && importRes.mappingStats.totalFields && importRes.mappingStats.mappedCount !== importRes.mappingStats.totalFields) {
          const allowedUnmappedByType = {
            post: ['ID', 'author_name', 'featured_image_title', 'featured_image_caption'],
            page: ['ID', 'author_name', 'featured_image_title', 'featured_image_caption'],
            custom_post_types: ['ID', 'author_name', 'featured_image_title', 'featured_image_caption'],
            woo_product: ['ID', 'author_name', 'featured_image_title', 'featured_image_caption'],
            woo_coupon: ['ID', 'author_name'],
            woo_order: ['ID', 'author_name'],
            taxonomy: ['parent_slug'],
            comment: ['post_permalink', 'post_type', 'post_slug'],
          };
          const allowed = new Set(allowedUnmappedByType[c.type] || []);
          const unmapped = Array.isArray(importRes.mappingStats.unmapped) ? importRes.mappingStats.unmapped : [];
          const important = unmapped.filter((x) => !allowed.has(String(x || '').trim()));
          if (important.length) {
        ...
        if (c.expectAcf) {
          const srcObjectId = String(exportInfo.sourceObjectId || '').trim();
          const targetObjectId = c.resolveTargetObjectId ? String(c.resolveTargetObjectId(exportInfo.identity || {}) || '').trim() : '';

          if (!srcObjectId) {
            result.status = 'issues';
            result.errors.push({ kind: 'acf-skip', reason: 'source-object-id-missing' });
          } else if (!targetObjectId) {
            result.status = 'issues';
            result.errors.push({ kind: 'acf-skip', reason: 'target-object-id-not-found', identity: exportInfo.identity || {} });
          } else {
            const expected = getAcfSnapshotForObject(env, env.source.wpPath, srcObjectId);
            const actual = getAcfSnapshotForObject(env, env.target.wpPath, targetObjectId);
            const mismatches = diffAcfSnapshots(expected, actual);
            result.acf = { sourceObjectId: srcObjectId, targetObjectId, mismatchesCount: mismatches.length };
            if (mismatches.length) {
              result.status = 'issues';
              result.errors.push({ kind: 'acf-mismatch', mismatches: mismatches.slice(0, 50) });
              fs.writeFileSync(path.join(caseDir, 'acf-expected.json'), JSON.stringify(expected, null, 2));
              fs.writeFileSync(path.join(caseDir, 'acf-actual.json'), JSON.stringify(actual, null, 2));
              fs.writeFileSync(path.join(caseDir, 'acf-mismatches.json'), JSON.stringify(mismatches, null, 2));
            }
          }
        }
        ...
        if (c.repeatImportForDedup && baselineHash) {
          const targetAttBefore = countAttachments(env, env.target.wpPath);
          const targetHashBeforeCase = countAttachmentsByFileHash(env, env.target.wpPath, baselineHash);

          await importIntoTarget(targetPage, env.target, c.type, exportPath, {
            requiredSelectors: exp.required || exportInfo.requiredSelectors || {},
            importOptions: c.importOptions || {},
          });

          const targetAttAfter = countAttachments(env, env.target.wpPath);
          const targetHashAfter = countAttachmentsByFileHash(env, env.target.wpPath, baselineHash);

          result.mediaDedup = {
            md5: baselineHash,
            attachments: { before: targetAttBefore, after: targetAttAfter },
            hashCount: { before: targetHashBeforeCase, after: targetHashAfter },
          };

          if (targetAttAfter > targetAttBefore) {
            result.status = 'issues';
            result.errors.push({ kind: 'media-dup-attachments', before: targetAttBefore, after: targetAttAfter });
          }
          if (targetHashAfter > Math.max(1, targetHashBeforeCase)) {
            result.status = 'issues';
            result.errors.push({ kind: 'media-dup-hash', md5: baselineHash, before: targetHashBeforeCase, after: targetHashAfter });
          }
        }

        // Per-case screenshots (best-effort).
        await sourcePage.screenshot({ path: path.join(caseDir, 'source-last.png'), fullPage: true }).catch(() => {});
        await targetPage.screenshot({ path: path.join(caseDir, 'target-last.png'), fullPage: true }).catch(() => {});
      } catch (e) {
        result.status = 'error';
        result.errors.push({ kind: 'exception', message: String(e && e.message ? e.message : e) });
      }

      summary.results.push(result);
      fs.writeFileSync(path.join(caseDir, 'result.json'), JSON.stringify(result, null, 2));
      console.log(`[case] ${c.type} :: ${c.label} => ${result.status}${result.skipped ? ' (skipped)' : ''}`);
    }

    // Global dedup baseline stats (target) for visibility.
    const targetAttachmentsAfter = countAttachments(env, env.target.wpPath);
    const targetHashAfterAll = baselineHash ? countAttachmentsByFileHash(env, env.target.wpPath, baselineHash) : 0;
    summary.mediaBaseline = {
      md5: baselineHash,
      targetAttachments: { before: targetAttachmentsBefore, after: targetAttachmentsAfter },
      targetHashCount: { before: targetHashBefore, after: targetHashAfterAll },
    };

    // Summarize issues.
    summary.issues = summary.results.filter((r) => r.status !== 'ok' && r.status !== 'skipped');
    ...
    console.log(`[done] Summary: ${path.join(artifactsRoot, 'summary.json')}`);
  } finally {
