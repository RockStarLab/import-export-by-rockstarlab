/**
 * Manual E2E (Playwright): Blog Posts export/import check.
 *
 * Flow:
 * - PRO off: export all blog posts from aie.local, import into aie2.local.
 ...
 * - Compare every source blog post against target in browser: editor fields,
 *   ACF DOM values, SEO/Elementor-related form/meta inputs, featured image,
 *   editor content, and frontend body text.
 *
 * Usage:
 *   PLAYWRIGHT_BROWSERS_PATH=./e2e/.playwright-browsers node scripts/aie-blog-posts-import-export-check.js
 */
...
async function setPluginActive( page, site, pluginFile, active ) {
	...
	let row = await pluginRow( page, pluginFile );
	if ( ( await isPluginActive( row ) ) === active ) return;
...
async function selectPostContentType( page ) {
	const input = page
		.locator(
			'.rsl-ie-step-1.active input[name="content_type"][value="post"]'
		)
		.first();
...
async function exportBlogPosts( page, env, outDir, { proMode } ) {
	await gotoAdmin(
		page,
		env.source,
	...
	const exportPath = path.join(
		outDir,
		`${ proMode ? 'pro' : 'free' }-blog-posts.csv`
	);
...
async function importBlogPosts( page, env, csvPath, { proMode } ) {
	await gotoAdmin(
		page,
		env.target,
...
async function openSourcePostEdit( page, env, postId ) {
	await gotoAdmin(
		page,
		env.source,
...
async function openTargetPostEditByTitle( page, env, title, slug = '' ) {
	for ( const term of [ title, slug ].filter( Boolean ) ) {
		await gotoAdmin(
			page,
			env.target,
			`/wp-admin/edit.php?post_type=post&s=${ encodeURIComponent(
				term
			) }`
		);
		...
		if ( await exactTitle.count() ) {
			...
			await assertPageHealthy( page, `target post edit ${ title }` );
			return;
		}
...
`
	...
	throw new Error(
		`Target post not found in browser by title/slug: ${ title } / ${ slug }`
	);
}
...
async function compareAllPostsInBrowser(
	sourcePage,
	targetPage,
	frontPage,
	env,
	...
	for ( const post of posts ) {
		const dir = path.join( outDir, `${ label }-post-${ post.ID }` );
		mkdirp( dir );
		await openSourcePostEdit( sourcePage, env, post.ID );
		...
		await openTargetPostEditByTitle(
			targetPage,
			env,
...
async function runIteration( ctx, env, posts, outRoot, { proMode } ) {
	...
	const exported = await exportBlogPosts( sourcePage, env, outDir, {
		proMode,
	} );
	const imported = await importBlogPosts(
		targetPage,
		env,
		exported.exportPath,
		{ proMode }
	);
	const comparisons = await compareAllPostsInBrowser(
		sourcePage,
		targetPage,
		frontPage,
		env,
...
async function run() {
	const env = loadEnv();
	const posts = listSourcePosts( env );
	if ( ! posts.length ) throw new Error( 'No source blog posts found' );
	const outRoot = path.resolve(
		process.cwd(),
		'e2e/artifacts/blog-posts-import-export',
		nowStamp()
	);
	mkdirp( outRoot );
	const summary = {
