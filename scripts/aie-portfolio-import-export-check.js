/**
 * Manual E2E (Playwright): Portfolio CPT export/import check.
 *
 * Flow:
 * - PRO on: export all portfolio items from aie.local, import into aie2.local.
 * - Restore aie2 from db.sql before each import iteration.
 * - Compare every source portfolio item against target in browser: editor fields,
 *   ACF DOM values, SEO/Elementor-related form/meta inputs, featured image,
 *   editor content, and frontend body text.
 *
 * Usage:
 *   PLAYWRIGHT_BROWSERS_PATH=./e2e/.playwright-browsers node scripts/aie-portfolio-import-export-check.js
 */
...
function listSourcePortfolio( env ) {
	const php = `
$posts = get_posts([
  'post_type' => 'portfolio',
  'post_status' => 'any',
...
async function selectPortfolioContentType( page ) {
	const input = page
		.locator(
			'.rsl-ie-step-1.active input[name="content_type"][value="custom_post_types"]'
		)
		.first();
	...
	await page.evaluate( () => {
		...
		if ( ! el ) throw new Error( 'Portfolio content type radio not found' );
		el.checked = true;
...
async function selectPortfolioFilter( page ) {
	await page.locator( '.rsl-ie-step-2.active .rsl-ie-add-filter' ).click();
	const row = page
		.locator( '.rsl-ie-step-2.active .rsl-ie-filter-row' )
		.last();
	await row.waitFor( { state: 'visible', timeout: 30_000 } );
	const field = row.locator( '.rsl-ie-filter-field' );
	await field.selectOption( '_post_type' );
	await row
		.locator( '.rsl-ie-post-type-selector' )
	...
	await page
		.waitForFunction(
			() => {
				...
				return count > 0 && next && ! next.disabled;
			},
			null,
			{ timeout: 60_000 }
		)
...
async function exportPortfolio( page, env, outDir, { proMode } ) {
	await gotoAdmin(
		page,
		env.source,
	...
	await selectPortfolioContentType( page );
	...
	await page
		...
		.click()
		.catch( () => null );
	await selectPortfolioFilter( page );
	await clickNextStep( page );
	...
	const exportPath = path.join(
		outDir,
		`${ proMode ? 'pro' : 'free' }-portfolio.csv`
	);
...
async function importPortfolio( page, env, csvPath, { proMode } ) {
	await gotoAdmin(
		page,
		env.target,
	...
	await page
		.locator( '#rsl-ie-custom-post-type' )
		.selectOption( 'portfolio' )
		.catch( () => null );
	await page
...
async function openSourcePortfolioEdit( page, env, postId ) {
	await gotoAdmin(
		page,
		env.source,
...
async function openTargetPortfolioEditByTitle( page, env, title, slug = '' ) {
	for ( const term of [ title, slug ].filter( Boolean ) ) {
		await gotoAdmin(
			page,
			env.target,
			`/wp-admin/edit.php?post_type=portfolio&s=${ encodeURIComponent(
				term
			) }`
		);
		...
		if ( await exactTitle.count() ) {
			...
			await assertPageHealthy( page, `target portfolio edit ${ title }` );
			return;
		}
...
$p = get_page_by_path($slug, OBJECT, 'portfolio');
if (!$p) {
  global $wpdb;
  $p = $wpdb->get_row($wpdb->prepare(
    "SELECT * FROM {$wpdb->posts} WHERE post_type = 'portfolio' AND post_status <> 'trash' AND (post_name = %s OR post_title = %s) ORDER BY ID ASC LIMIT 1",
    $slug,
    $title
  ));
}
...
`
	...
	throw new Error(
		`Target portfolio item not found in browser by title/slug: ${ title } / ${ slug }`
	);
}
...
async function extractFrontendSnapshot( page, site, browserSnapshot ) {
	...
	const postType = browserSnapshot.editor.type || '';
	const pathPrefix =
		postType && ! [ 'page', 'post' ].includes( postType )
			? `/${ postType }`
			: '';
	const url = `${ site.baseUrl.replace(
		/\/$/,
		''
	) }${ pathPrefix }/${ slug }/`;
...
async function compareAllPortfolioInBrowser(
	sourcePage,
	targetPage,
	frontPage,
	env,
	...
	for ( let index = 0; index < posts.length; index++ ) {
		...
		const dir = path.join( outDir, `${ label }-portfolio-${ post.ID }` );
		mkdirp( dir );
		fs.writeFileSync(
		...
		await openSourcePortfolioEdit( sourcePage, env, post.ID );
		...
		await openTargetPortfolioEditByTitle(
			targetPage,
			env,
...
async function runIteration( ctx, env, posts, outRoot, { proMode } ) {
	...
	console.log( `[${ label }] export portfolio` );
	const exported = await exportPortfolio( sourcePage, env, outDir, {
		proMode,
	} );
	console.log( `[${ label }] import portfolio` );
	const imported = await importPortfolio(
		targetPage,
		env,
		exported.exportPath,
		{
	...
	console.log( `[${ label }] compare portfolio` );
	const comparisons = await compareAllPortfolioInBrowser(
		sourcePage,
		targetPage,
		frontPage,
		env,
...
async function run() {
	const env = loadEnv();
	const posts = listSourcePortfolio( env );
	if ( ! posts.length ) throw new Error( 'No source portfolio items found' );
	const outRoot = path.resolve(
		process.cwd(),
		'e2e/artifacts/portfolio-import-export',
		nowStamp()
	);
	mkdirp( outRoot );
	const summary = {
