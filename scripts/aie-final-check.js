...
function loadEnv() {
  ...
  return {
    types,
    tag,
    taxonomy: String(get('AIE_TAXONOMY', 'category')).trim() || 'category',
    customPostType: String(get('AIE_CUSTOM_POST_TYPE', 'product')).trim() || 'product',
    dbTablePatterns: String(get('AIE_DB_TABLE_PATTERNS', 'otbo,mask')),
    sourceWpPath: String(get('AIE_SOURCE_WP_PATH', sourceWpPathDefault)),
    targetWpPath: String(get('AIE_TARGET_WP_PATH', targetWpPathGuess || sourceWpPathDefault)),
  };
}

function sh(cmd, opts = {}) {
  return execFileSync('/bin/zsh', ['-lc', cmd], { encoding: 'utf8', ...opts }).trim();
}
...
function appendPostContent({ wpPath, postId, marker }) {
  const content = wp({ wpPath, args: ['post', 'get', String(postId), '--field=post_content'] });
  const updated = `${content}\n\n${marker}\n`;
  wp({ wpPath, args: ['post', 'update', String(postId), `--post_content=${updated}`] });
}

function updatePostMeta({ wpPath, postId, key, value }) {
  wp({ wpPath, args: ['post', 'meta', 'update', String(postId), key, value] });
}

function setFeaturedImageToNextAttachment({ wpPath, postId }) {
  const idsRaw = wp({ wpPath, args: ['post', 'list', '--post_type=attachment', '--posts_per_page=5', '--format=ids'] });
  const ids = idsRaw
    .split(/\s+/)
    .map((x) => Number(x))
  ...
  const currentRaw = safe(() => wp({ wpPath, args: ['post', 'meta', 'get', String(postId), '_thumbnail_id'] }));
  const current = currentRaw.ok ? Number(String(currentRaw.value || '').trim()) : 0;
  const chosen = ids.find((id) => id && id !== current) || ids[0];
  wp({ wpPath, args: ['post', 'meta', 'update', String(postId), '_thumbnail_id', String(chosen)] });
}
...
function mutateSourceForType(env, type) {
  ...
  if (type === 'post') {
    const postId = Number(wp({ wpPath, args: ['post', 'list', '--post_type=post', '--posts_per_page=1', '--format=ids'] }).split(/\s+/)[0] || 0);
    ...
    updatePostMeta({ wpPath, postId, key: 'repeater_0_text', value: marker });
  ...
  if (type === 'page') {
    const idsRaw = wp({ wpPath, args: ['post', 'list', '--post_type=page', '--posts_per_page=3', '--format=ids'] });
    const ids = idsRaw
      .split(/\s+/)
      .map((x) => Number(x))
      .filter((n) => Number.isFinite(n) && n > 0);
  ...
  if (type === 'woo_product') {
    const postId = Number(
      wp({ wpPath, args: ['post', 'list', '--post_type=product', '--posts_per_page=1', '--format=ids'] }).split(/\s+/)[0] || 0
    );
    if (!postId) return { skipped: true, reason: 'No products found' };
    appendPostContent({ wpPath, postId, marker });
    updatePostMeta({ wpPath, postId, key: '_aie_final_marker', value: marker });
    setFeaturedImageToNextAttachment({ wpPath, postId });
    mutateAcfFieldsBestEffort({ wpPath, postId, marker });
  ...
  if (type === 'woo_order') {
    let postId = Number(
      wp({ wpPath, args: ['post', 'list', '--post_type=shop_order', '--posts_per_page=1', '--format=ids'] }).split(/\s+/)[0] || 0
    );
    ...
    if (!postId) {
      const php = [
        ...
        '$order->set_customer_note(' + JSON.stringify(String(marker)) + ');',
        '$order->calculate_totals();',
        '$order->save();',
        'echo (int) $order->get_id();',
      ].join(' ');
      const created = safe(() => wp({ wpPath, args: ['eval', php] }));
      postId = created.ok ? Number(String(created.value || '').trim() || 0) : 0;
    }

    if (!postId) return { skipped: true, reason: 'No orders found and could not create one' };
    updatePostMeta({ wpPath, postId, key: '_customer_note', value: marker });
    updatePostMeta({ wpPath, postId, key: '_aie_final_marker', value: marker });
  ...
  if (type === 'woo_coupon') {
    const postId = Number(
      wp({ wpPath, args: ['post', 'list', '--post_type=shop_coupon', '--posts_per_page=1', '--format=ids'] }).split(/\s+/)[0] || 0
    );
    if (!postId) return { skipped: true, reason: 'No coupons found' };
    appendPostContent({ wpPath, postId, marker });
    updatePostMeta({ wpPath, postId, key: '_aie_final_marker', value: marker });
    mutateAcfFieldsBestEffort({ wpPath, postId, marker });
    return { couponId: postId };
  }

  if (type === 'media') {
    const attachmentId = Number(
      wp({ wpPath, args: ['post', 'list', '--post_type=attachment', '--posts_per_page=1', '--format=ids'] }).split(/\s+/)[0] || 0
    );
    if (!attachmentId) return { skipped: true, reason: 'No attachments found' };
    wp({ wpPath, args: ['post', 'update', String(attachmentId), `--post_title=${marker}`] });
    wp({ wpPath, args: ['post', 'meta', 'update', String(attachmentId), '_wp_attachment_image_alt', marker] });
    return { attachmentId };
  }

  if (type === 'menu') {
    // Best-effort: update the first menu item's title in the first menu.
    const menuName = wp({ wpPath, args: ['menu', 'list', '--fields=name', '--format=csv'] })
      .split(/\r?\n/)
      .slice(1)
      .map((x) => x.trim())
      .filter(Boolean)[0];
    ...
    const itemIds = wp({ wpPath, args: ['menu', 'item', 'list', menuName, '--fields=db_id', '--format=csv'] })
      .split(/\r?\n/)
      .slice(1)
      .map((x) => Number(x.trim()))
      .filter((n) => Number.isFinite(n) && n > 0);
    const firstItem = itemIds[0];
    if (!firstItem) return { skipped: true, reason: 'Menu has no items' };

    wp({ wpPath, args: ['menu', 'item', 'update', String(firstItem), `--title=${marker}`] });
    return { menuName, menuItemId: firstItem };
  }

  if (type === 'user') {
    const userId = Number(
      wp({ wpPath, args: ['user', 'list', '--role=administrator', '--number=1', '--field=ID'] }).split(/\s+/)[0] || 0
    );
    if (!userId) return { skipped: true, reason: 'No admin users found' };
    wp({ wpPath, args: ['user', 'meta', 'update', String(userId), 'first_name', marker] });
    return { userId };
  }

  if (type === 'comment') {
    const commentId = Number(
      wp({ wpPath, args: ['comment', 'list', '--number=1', '--field=comment_ID'] }).split(/\s+/)[0] || 0
    );
    if (!commentId) return { skipped: true, reason: 'No comments found' };
    wp({ wpPath, args: ['comment', 'update', String(commentId), `--comment_content=${marker}`] });
    return { commentId };
  }
  ...
  if (type === 'taxonomy') {
    ...
    const termId = Number(
      wp({ wpPath, args: ['term', 'list', taxonomy, '--number=1', '--field=term_id'] }).split(/\s+/)[0] || 0
    );
    if (!termId) return { skipped: true, reason: `No terms found in taxonomy=${taxonomy}` };
    wp({ wpPath, args: ['term', 'update', taxonomy, String(termId), `--description=${marker}`] });
  ...
  if (type === 'woo_attribute') {
    ...
    if (!attributeId) return { skipped: true, reason: 'No Woo attributes found' };
    return { attributeId };
  }
  ...
  if (type === 'database_table') {
    const patterns = env.dbTablePatterns
      ...
      .map((x) => x.trim())
      .filter(Boolean);
    const jsPatterns = patterns.map((p) => p.replace(/'/g, '')).join(',');
    ...
    const res = wp({
      ...
      args: [
        ...
        [
          'global $wpdb;',
          '$patterns = array_filter(array_map("trim", explode(",", "' + jsPatterns + '")));',
          ...
          'if ($setCol) { $wpdb->update($pick, [$setCol => "AIE FINAL ' + env.tag.replace(/'/g, '') + '"], [$whereCol => $whereVal]); }',
          'echo $pick;',
        ].join(' '),
      ],
    });
    const tableName = String(res || '').trim();
    if (!tableName) return { skipped: true, reason: `No non-empty table matched patterns=${env.dbTablePatterns}` };
    return { tableName };
  }
  ...
  if (type === 'custom_post_types') {
    ...
    const postId = Number(
      wp({ wpPath, args: ['post', 'list', `--post_type=${postType}`, '--posts_per_page=1', '--format=ids'] }).split(/\s+/)[0] || 0
    );
    if (!postId) return { skipped: true, reason: `No posts found for post_type=${postType}` };
    appendPostContent({ wpPath, postId, marker });
    updatePostMeta({ wpPath, postId, key: '_aie_final_marker', value: marker });
    mutateAcfFieldsBestEffort({ wpPath, postId, marker });
...
function runVisualMulti(env, type, extraEnv = {}) {
  const nodeEnv = {
    ...
    AIE_MEDIA_DUPLICATE_MODE: process.env.AIE_MEDIA_DUPLICATE_MODE || 'skip',
    ...extraEnv,
  };

  const result = spawnSync('node', ['scripts/aie-import-export-visual-multi.js'], {
    cwd: process.cwd(),
    env: nodeEnv,
    stdio: 'inherit',
  });
  if (result.status !== 0) {
    throw new Error(`visual-multi failed for type=${type} (exit=${result.status})`);
  }
}
...
function getSourceIdsForNonPostType({ env, type, mutateResult }) {
  ...
  if (type === 'media') {
    if (mutateResult && mutateResult.attachmentId) return [Number(mutateResult.attachmentId)].filter(Boolean);
    const idsRaw = safe(() =>
      wp({ wpPath, args: ['post', 'list', '--post_type=attachment', '--posts_per_page=3', '--format=ids'] })
    );
  ...
  if (type === 'user') {
    if (mutateResult && mutateResult.userId) return [Number(mutateResult.userId)].filter(Boolean);
    const idsRaw = safe(() => wp({ wpPath, args: ['user', 'list', '--number=3', '--field=ID'] }));
    if (idsRaw.ok && idsRaw.value) {
      return String(idsRaw.value)
        .trim()
        .split(/\s+/)
  ...
  if (type === 'comment') {
    if (mutateResult && mutateResult.commentId) return [Number(mutateResult.commentId)].filter(Boolean);
    const idsRaw = safe(() => wp({ wpPath, args: ['comment', 'list', '--number=3', '--field=comment_ID'] }));
    if (idsRaw.ok && idsRaw.value) {
      return String(idsRaw.value)
        .trim()
        .split(/\s+/)
  ...
  if (type === 'taxonomy') {
    if (mutateResult && mutateResult.termId) return [Number(mutateResult.termId)].filter(Boolean);
    const taxonomy = env.taxonomy || 'category';
    const idsRaw = safe(() => wp({ wpPath, args: ['term', 'list', taxonomy, '--number=3', '--field=term_id'] }));
    if (idsRaw.ok && idsRaw.value) {
      return String(idsRaw.value)
        .trim()
        .split(/\s+/)
...
async function main() {
  ...
  for (const type of env.types) {
    ...
    if (!mut.ok) {
      console.log(`[final] mutate failed (${type}): ${String(mut.error && mut.error.message ? mut.error.message : mut.error)}`);
    } else if (mut.value && mut.value.skipped) {
      console.log(`[final] mutate skipped (${type}): ${mut.value.reason || 'n/a'}`);
    } else {
      console.log(`[final] mutate ok (${type}): ${JSON.stringify(mut.value || {})}`);
    }

    const extraEnv = {};
    if (type === 'database_table' && mut.ok && mut.value && mut.value.tableName) {
    ...
    if (type === 'woo_order' && mut.ok && mut.value && mut.value.orderId) {
      extraEnv.AIE_SOURCE_POST_IDS = String(mut.value.orderId);
      console.log(`[final] compare IDs (${type}): ${extraEnv.AIE_SOURCE_POST_IDS}`);
    }
    ...
    if (comparePostType && !extraEnv.AIE_SOURCE_POST_IDS) {
      const idsRaw = safe(() =>
        wp({
          ...
          args: ['post', 'list', `--post_type=${comparePostType}`, '--post_status=any', '--posts_per_page=3', '--format=ids'],
        })
      );
      if (idsRaw.ok && idsRaw.value) {
        extraEnv.AIE_SOURCE_POST_IDS = String(idsRaw.value).trim().split(/\s+/).slice(0, 3).join(',');
        console.log(`[final] compare IDs (${type}): ${extraEnv.AIE_SOURCE_POST_IDS}`);
      }
    } else {
      const ids = getSourceIdsForNonPostType({ env, type, mutateResult: mut.ok ? mut.value : null });
      if (ids.length) {
        extraEnv.AIE_SOURCE_POST_IDS = ids.join(',');
        console.log(`[final] compare IDs (${type}): ${extraEnv.AIE_SOURCE_POST_IDS}`);
      }
    }

    const visual = safe(() => runVisualMulti(env, type, extraEnv));
    if (!visual.ok) {
      console.log(`[final] visual FAILED (${type}): ${String(visual.error && visual.error.message ? visual.error.message : visual.error)}`);
      results.push({ type, ok: false, error: String(visual.error && visual.error.message ? visual.error.message : visual.error) });
      continue;
    }
  ...
  if (failed.length) {
    console.log(`[final] FAILURES: ${failed.map((f) => f.type).join(', ')}`);
    process.exitCode = 2;
  } else {
