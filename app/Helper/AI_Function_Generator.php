...
class AI_Function_Generator {
	...
	public function generate_function( $prompt ) {
		if ( empty( $this->api_key ) ) {
			return new \WP_Error(
				'no_api_key',
				__( 'OpenAI API key is not configured. Please add it to wp-config.php as WP_AIE_OPENAI_API_KEY or configure it in settings.', 'wp-advanced-import-export' )
			);
		}
...
- Return response as JSON with name, description, and code',
	...
	private function call_openai_api( $system_prompt, $user_prompt ) {
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Failed to connect to OpenAI API: %s', 'wp-advanced-import-export' ),
		...
		if ( 200 !== $response_code ) {
			...
			$error_message = $error_data['error']['message'] ?? __( 'Unknown API error', 'wp-advanced-import-export' );

			return new \WP_Error(
				'api_error',
				sprintf(
					/* translators: 1: HTTP code, 2: error message */
					__( 'OpenAI API returned error %1$d: %2$s', 'wp-advanced-import-export' ),
					$response_code,
					$error_message
				)
			);
	...
	private function parse_response( $response ) {
		...
		if ( json_last_error() !== JSON_ERROR_NONE ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Failed to parse AI response: %s', 'wp-advanced-import-export' ),
					json_last_error_msg()
				)
			);
		}
		...
		if ( ! isset( $data['code'] ) || empty( $data['code'] ) ) {
			return new \WP_Error(
				'missing_code',
				__( 'AI did not generate valid code', 'wp-advanced-import-export' )
			);
		}
		...
		if ( stripos( $code, 'return' ) === false ) {
			return new \WP_Error(
				'invalid_code',
				__( 'Generated code must contain a return statement', 'wp-advanced-import-export' )
			);
		}
