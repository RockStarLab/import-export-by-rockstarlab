	...
	public static function get_handler_by_extension( $extension ) {
		...
		return new \WP_Error(
			...
			sprintf(
				...
				__( 'Unsupported file format: %s', 'wp-advanced-import-export' ),
				$extension
			)
		);
	}
	...
	public static function get_handler_by_mime( $mime_type ) {
		...
		return new \WP_Error(
			...
			sprintf(
				...
				__( 'Unsupported MIME type: %s', 'wp-advanced-import-export' ),
				$mime_type
			)
		);
	}
		...
		if ( ! isset( $class_map[ $format ] ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Invalid format: %s', 'wp-advanced-import-export' ),
					$format
				)
			);
		}
