...
class Comment_Importer extends Abstract_Importer {
	...
	public function prepare( $data, $mapping = [] ) {
		...
		foreach ( $data as $row ) {
			...
			foreach ( array_merge( $this->get_required_fields(), $this->get_optional_fields() ) as $field ) {
				if ( ( ! isset( $item[ $field ] ) || '' === $item[ $field ] ) && isset( $row[ $field ] ) ) {
	...
	public function import_item( $item, $index ) {
		...
		$source_comment_id     = absint( $item['_aie_source_comment_id'] ?? ( $item['source_comment_id'] ?? ( $item['comment_ID'] ?? 0 ) ) );
		$source_comment_parent = absint( $item['_aie_source_comment_parent_id'] ?? ( $item['source_comment_parent_id'] ?? ( $item['comment_parent'] ?? 0 ) ) );
		...
		if ( ! $post_id ) {
			if ( $this->get_option( 'create_missing_posts', false ) ) {
				$new_post_id = $this->create_placeholder_post_for_comment( $item );
				if ( is_wp_error( $new_post_id ) ) {
					return $new_post_id;
				}

				$post_id                 = absint( $new_post_id );
				$item['comment_post_ID'] = $post_id;
			}
		}
		...
		if ( ! $post_id ) {
			return new \WP_Error(
				'missing_post_id',
				__( 'Could not match the comment to a post. Map Post Slug, Post Permalink, Source Post ID, Post Title, or a valid local Post ID.', 'import-export-by-rockstarlab' )
			);
		}
	...
	private function update_comment( $comment_id, $item, $source_comment_id = 0 ) {
		...
		// Update metadata
		$this->import_comment_meta( $comment_id, $item );
		$this->store_source_comment_meta( $comment_id, $source_comment_id, absint( $item['_aie_source_comment_parent_id'] ?? ( $item['source_comment_parent_id'] ?? 0 ) ) );
	...
	private function has_post_hints( $item ) {
		...
		$source_id = absint( $item['_aie_source_post_id'] ?? ( $item['source_post_id'] ?? 0 ) );
		$title     = isset( $item['post_title'] ) ? (string) $item['post_title'] : '';

		return $source_id > 0 || trim( $permalink ) !== '' || trim( $slug ) !== '' || trim( $type ) !== '' || trim( $title ) !== '';
	}
	...
	private function resolve_comment_post_id( $item ) {
		...
		$source_post_id = absint( $item['_aie_source_post_id'] ?? ( $item['source_post_id'] ?? 0 ) );
		if ( $source_post_id > 0 ) {
			$matched_by_source = $this->find_post_by_source_id( $source_post_id );
			if ( $matched_by_source > 0 ) {
	...
	private function find_post_by_source_id( $source_post_id ) {
		...
		$matched = get_posts(
			[
				...
				'meta_key'               => '_rsl_ie_source_post_id', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key -- Imported post source-ID lookup.
				'meta_value'             => $source_post_id, // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_value -- Exact source post ID match.
			]
		);

		return ! empty( $matched[0] ) ? absint( $matched[0] ) : 0;
	}
	...
	/**
	 * Create a placeholder post when a source comment references content that is
	 * not present on the target site.
	 *
	 * @param array $item Prepared comment row.
	 * @return int|\WP_Error
	 */
	private function create_placeholder_post_for_comment( $item ) {
		$post_type  = isset( $item['post_type'] ) && post_type_exists( $item['post_type'] )
			? sanitize_key( $item['post_type'] )
			: 'post';
		$post_title = isset( $item['post_title'] ) && '' !== trim( (string) $item['post_title'] )
			? sanitize_text_field( $item['post_title'] )
			: __( 'Imported Comment Source', 'import-export-by-rockstarlab' );
		$post_name  = isset( $item['post_slug'] ) ? sanitize_title( basename( trim( (string) $item['post_slug'], '/' ) ) ) : '';
	...
	private function import_acf_fields( $comment_id, $item ) {
		...
		foreach ( $item as $field_name => $field_value ) {
			if ( strpos( $field_name, 'acf_' ) === 0 && '' !== $field_value ) {
				$this->import_acf_value( $comment_id, substr( $field_name, 4 ), $field_value );
			}
		}
	...
	/**
	 * Import one comment ACF value, optionally downloading portable media.
	 *
	 * @param int    $comment_id  Comment ID.
	 ...
	 * @return void
	 */
	private function import_acf_value( $comment_id, $field_name, $field_value ) {
		$comment_id = absint( $comment_id );
		$field_name = (string) $field_name;
		if ( $comment_id <= 0 || '' === $field_name ) {
			return;
		}
		...
		if ( function_exists( 'update_field' ) ) {
			update_field( $field_name, $field_value, ACF_Fields::get_acf_object_id( 'comment', $comment_id ) );
		}
	}
