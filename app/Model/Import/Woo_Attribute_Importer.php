...
class Woo_Attribute_Importer extends Abstract_Importer {
	...
	public function get_description() {
		return __( 'Import WooCommerce product attributes with terms', 'wp-advanced-import-export' );
	}
	...
	public function get_available_fields() {
		return [
			'attribute_name'    => [
				'label'       => __( 'Attribute Name', 'wp-advanced-import-export' ),
				'description' => __( 'Unique attribute slug (e.g., "color", "size")', 'wp-advanced-import-export' ),
				'required'    => true,
			],
			'attribute_label'   => [
				'label'       => __( 'Attribute Label', 'wp-advanced-import-export' ),
				'description' => __( 'Human-readable label (e.g., "Color", "Size")', 'wp-advanced-import-export' ),
			],
			'attribute_type'    => [
				'label'       => __( 'Attribute Type', 'wp-advanced-import-export' ),
				'description' => __( 'Type of attribute (select, text)', 'wp-advanced-import-export' ),
			],
			'attribute_orderby' => [
				'label'       => __( 'Order By', 'wp-advanced-import-export' ),
				'description' => __( 'Default sort order (menu_order, name, name_num, id)', 'wp-advanced-import-export' ),
			],
			'attribute_public'  => [
				'label'       => __( 'Enable Archives', 'wp-advanced-import-export' ),
				'description' => __( 'Enable archives for this attribute (0 or 1)', 'wp-advanced-import-export' ),
			],
			'attribute_terms'   => [
				'label'       => __( 'Attribute Terms', 'wp-advanced-import-export' ),
				'description' => __( 'JSON array of terms with name, slug, and description OR comma-separated list of term names', 'wp-advanced-import-export' ),
			],
		];
	}
	...
	public function get_supported_options() {
		return [
			'update_existing'     => __( 'Update existing attributes if found', 'wp-advanced-import-export' ),
			'update_terms'        => __( 'Update existing terms if found', 'wp-advanced-import-export' ),
			'skip_existing_terms' => __( 'Skip existing terms (don\'t update them)', 'wp-advanced-import-export' ),
		];
	}
	...
	public function import_item( $item, $index ) {
		...
		if ( empty( $item['attribute_name'] ) ) {
			return new \WP_Error(
				...
				__( 'Attribute name is required', 'wp-advanced-import-export' )
			);
		}
		...
		if ( false === $result ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Failed to create attribute: %s', 'wp-advanced-import-export' ),
					$attribute_name
				)
			);
		}
	...
	protected function update_attribute( $attribute_id, $item ) {
		...
		if ( false === $result ) {
			return new \WP_Error(
				...
				sprintf(
					/* translators: %s: attribute name */
					__( 'Failed to update attribute: %s', 'wp-advanced-import-export' ),
					$attribute_name
				)
			);
		}
